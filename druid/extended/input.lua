...
function Input.init(self, click_node, text_node, keyboard_type)
	...
	self.is_multiline = gui.get_line_break(self:get_node(text_node)) --sub
	self.symbols_per_line =  math.ceil(gui.get_size(self:get_node(text_node)).x / self.text:get_text_size("n")) --sub
	self.text_bounds = gui.get_size(self:get_node(self.text_node))
	self.margin = 6 -- зазор до стенок, если нужен
	self.pivot = gui.get_pivot(self:get_node(self.text_node))
	self.click_node = click_node
	
	if self.is_multiline then --отключаем автоперенос, будем мониторить это дело исключительно вручную
		gui.set_line_break(self:get_node(text_node), false)
	end
...
function Input.on_input(self, action_id, action)
	...
	if self.is_selected then
		...
		if action_id == const.ACTION_TEXT then
			...
			if not utf8.match(hex, "EF9C8[0-3]") then
				if not self.allowed_characters or utf8.match(action.text, self.allowed_characters) then
					...
					local add_count = utf8.len(action.text)  --сколько символов добавилось (если тыкать кнопки враз, то здесь сразу по 2-3 символа может быть)
					---------------------------
					if self.is_multiline then
						--local cur_string = utf8.match(prefix, ".*\n(.*)") or prefix
						local prefix_string, cur_string = utf8.match(prefix, "(.*\n)(.*)") 
						if not cur_string then
							cur_string = prefix
						end
						
						--local len = utf8.len(cur_string)
						--if len >= self.symbols_per_line then
						--print ("qq2d", self.text:get_text_size(cur_string),gui.get_size(self:get_node(self.text_node)).x)
						if self.text:get_text_size(cur_string) >= (self.text_bounds.x - self.margin) then
							prefix = prefix .. "\n"
							add_count = add_count + 1  --кроме того что есть, введен ещё один
						end
					end
					----------------------------
					...
					self.cursor_letter_index = self.cursor_letter_index + add_count 
					
					if self.max_length then
						input_text = utf8.sub(input_text, 1, self.max_length)
					end
					--print ("qq2d", self.cursor_letter_index, self.text:get_text_size(cur_string))
				else
		...
		if action_id == const.ACTION_ENTER and action.pressed then
			if self.is_multiline then
				local prefix = utf8.sub(self.value, 1, self.cursor_letter_index)
				local add_count = 1 --сколько символов добавилось (было 2)
				local prefix_string, cur_string = utf8.match(prefix, "(.*\n)(.*)") 
				if not cur_string then
					cur_string = prefix
				end
				prefix = prefix .. "\n"  --вот тут был пробел после переноса
				local postfix =  utf8.sub(self.value,  self.cursor_letter_index + 1)
				input_text =prefix .. postfix
