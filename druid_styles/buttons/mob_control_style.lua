...
M["button"] = {
	...
	DISABLED_COLOR = vmath.vector4(1, 1, 1, 1),
    ...
    gui.play_flipbook(node, state and "gui_control_button_push" or "gui_control_button_cursor")
		
		local tree = gui.get_tree(node)
		for k,v in pairs(tree) do
			if v ~= node then
				gui.set_color(v, state and vmath.vector4(155/255, 140/255, 125/255, 1) or vmath.vector4(103/255, 93/255, 83/255,1))
			end
		end
	end, 
	...
	on_mouse_hover = function(self, node, state)
		...
		gui.play_flipbook(node, state and "gui_control_button_cursor" or "gui_control_button_default")
	end,
	...
	on_click = function(self, node)
		...
		-- local tree = gui.get_tree(node) -- на клике иконку надо покрасить в ккнопке
		-- for k,v in pairs(tree) do
		-- 	if v ~= node then
		-- 		gui.set_color(v, vmath.vector4(155/255, 140/255, 125/255, 1))
		-- 	end
		-- end
	end,
	...
	on_click_disabled = function(self, node)
		...
		-- for k,v in pairs(tree) do
		-- 	if v ~= node then
		-- 		gui.set_color(v, vmath.vector4(103/255, 93/255, 83/255,1))
		-- 	end
		-- end
	end,
